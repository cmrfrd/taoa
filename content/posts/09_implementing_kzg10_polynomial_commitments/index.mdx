---
title: "Implementing KZG10 Polynomial Commitments"
...
There are many PCSs like [FRI](https://drops.dagstuhl.de/opus/volltexte/2018/9018/pdf/LIPIcs-ICALP-2018-14.pdf) or [IPA](https://eprint.iacr.org/2017/1066.pdf) (used in bulletproofs), but KZG10
...
[zkSNARKs](https://vitalik.ca/general/2021/01/26/snarks.html)), constant space and time is pretty neat! However the largest
criticism / drawback of KZG10 is that it requires a [trusted](https://zkproof.org/2021/06/30/setup-ceremonies/) [setup](https://vitalik.ca/general/2022/03/14/trustedsetup.html).
...
unknown integer number (at least it's supposed to be). Unlike in [ECC public
key cryptography](https://cryptobook.nakov.com/asymmetric-key-ciphers/elliptic-curve-cryptography-ecc) where the key holder knows their private and public key (
$\left({\alpha}, g^{{\alpha}}\right)$ respectively), in KZG10 we have a bunch of
"public keys" with an "unknown" private key. Even though we don't know what
mystical number ${\alpha}$ was used to create these "public
keys", we do know that each successive "public key" is another successive
power of ${\alpha}$ (We will use this to our advantage when we
start talking about polynomials).


#### Why is the CRS secure?

Ensuring ${\alpha}$ is a secret is important for the security of
KZG10. If we knew ${\alpha}$ then we could forge commitments and
...
contribute randomness to ${\alpha}$ so no one can reconstruct it
without collusion. The process for generating a CRS through MPC ceremonies
is a bit out of scope for this post, but these resources by [Vitalik Buterin](https://vitalik.ca/general/2022/03/14/trustedsetup.html)
and [Sean Bowe](https://eprint.iacr.org/2017/1050.pdf) are great places to learn more.
...
${\pi}$, they both must be changed together.
...
We see that polynomial $p\left({\alpha}\right) - p\left(x\right)$ will
always have a positive root at ${\alpha}$ and will always be
divisible by ${\alpha} - x$.
...
When given $\left(z, y\right)$ data, Lagrange polynomials are normal
...
Not to be confused with the [polynomial thats just the constant $0$](https://mathworld.wolfram.com/ZeroPolynomial.html), a zero
polynomial is a polynomial whose "zeros" (a.k.a roots) are defined by some
set of data points $\left(z_{0}, z_{1}, {...}, z_{k}\right)$. This can be expressed as:
...
$\left(z, y\right)$ points are legitimate evaluations of
...
a factor](https://sharmaeklavya2.github.io/theoremdep/nodes/polynomials/product-of-linear-factors-is-factor.html). We can now rest assured that this extension is correct because
...
"batching", the size of our proof ${\pi}$ stays the same (one
EC point)! Our only limitation is the size of the CRS which determines the
number of points we can verify (bounded in $g^{Z\left({\alpha}\right)}$ and
${\pi}$). And the size of the polynomial we can verify
(bounded in $C$).
