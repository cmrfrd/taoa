...
secret: false
excerpt: "Solving common problems with commitment schemes"
---
...
circumstance one of them is randomly chosen to become _the decider_ for that
...
in person and either flip a coin to see who becomes _the decider_, or they
...
and sends Bob [the paper](https://www.cs.cmu.edu/~mblum/research/pdf/coin/).
...
To learn more about random oracles check out [Matthew Green's blog post](https://blog.cryptographyengineering.com/2011/09/29/what-is-random-oracle-model-and-why-3/) about
it.
...
oracles) are impossible to construct due to the _pigeonhole principle_.
...
The role of the _Guesser_ will be played by Bob, and the role of
the _Flipper_ will be played by Alice. The ground rules / assumptions are:
...
3.  Both parties are talking over a secure channel that can't be intercepted
    or tampered with
4.  If the _Guesser_ correctly predicts the flip, they will become _the
    decider_. Otherwise, the _Flipper_ will become _the decider_
...
#### Cheating as the _Guesser_

In Bob's role as the _Guesser_ there isn't much room for cheating. The only
...
#### Cheating as the _Flipper_

In Alice's role as the _Flipper_ there are a few places where she can try to
...
-   Footnote: You might be wondering why Bob would need to generate an OTP at
    the start at all. Bob does this so he isn't vulnerable to a _pre-image
    attack_.
...
_the decider_ of their lunch spot. However it doesn't solve their problem of
...
Instead of a _Guesser_ and a _Flipper_, both will play the role of a _Roller_.
...
In order for a _Roller_ (we'll say Bob in this case) to successfully cheat,
...
Alice and Bob agree on this new _virtual die_ scheme so when they jointly need
to decide on a single spot from a set of restaurants, they break out the die
rolling protocol.

With the _virtual coin_ and _virtual die_ both of Alice and Bobs lunch
